...
State = tuple[Player, int] # Tuple of player (whose turn it is),
...
class Game:
    ...
    def utility(self, state: State, player: int) -> Utility:
...
def max_value(game: Game, state: State, player:Player) -> tuple[Utility, Action | None]:
...
def min_value(game: Game, state: State, player:Player) -> tuple[Utility, Action | None]:
    if game.is_terminal(state): return game.utility(state, player), None
    v, move = math.inf, None
    for a in game.actions(state):
